...
function retrieveEmployeeDetails () {
    accountant.value = localStorage.getItem("accountant") || ""
...
function saveEmployeeDetails () {
    accountant.value && localStorage.setItem("accountant", accountant.value)
...
<template>
    <form class="print:hidden h-screen w-auto flex flex-1 p-10 gap-10 justify-center items-center bg-slate-900 rounded-2xl shadow-2xl text-slate-100 italic overflow-y-auto">
        <fieldset class="flex flex-col gap-5 items-center">
            ...
            <button class="py-2 px-4 bg-slate-600 hover:bg-slate-500 font-semibold rounded shadow" @click="saveEmployeeDetails" type="submit">Sauvegarder les informations</button>
        </fieldset>
        <div class="flex flex-col gap-5">
            ...
            <footer class="flex flex-col gap-1 items-center">
